...
/**
 * Post a reply to an existing tweet via the X API (HITL — operator-approved).
 * Uses the same OAuth 1.0a credentials as postTweet.
 * Throws the raw error when not configured or on failure.
 */
export async function replyToTweet(
  ...
  if (!res.ok) {
    const body = await res.text();
    log?.warn({ status: res.status, body }, "X replyToTweet failed");
    throw new Error(`X replyToTweet failed: ${res.status}`);
  }
  const json = (await res.json()) as { data?: { id?: string } };
  const id = json.data?.id;
  if (!id) throw new Error("X replyToTweet returned no id");
  return {
    id,
    url: `https://x.com/${X_ACCOUNT_HANDLE}/status/${id}`,
    mode: "api",
